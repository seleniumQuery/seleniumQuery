...
public class NotEqualsAttributeSelectorFixTest {
	...
	public void fixAttributeNotEquals__should_change_attribute_not_equals_into_a_NOT_attribute_equals() {
		...
		// when
		String output = notEqualsAttributeSelectorFix.fixAttributeNotEquals(input);
		// then
	...
	public void fixAttributeNotEquals__2() {
		assertThat(notEqualsAttributeSelectorFix.fixAttributeNotEquals("a[rel!='nofollow']"), is(equalTo("a:not([rel='nofollow'])")));
		assertThat(notEqualsAttributeSelectorFix.fixAttributeNotEquals("div:contains([rel!='nofollow'])"), is(equalTo("div:contains([rel!='nofollow'])")));
		assertThat(notEqualsAttributeSelectorFix.fixAttributeNotEquals("div:contains(\"[rel!='nofollow']\")"), is(equalTo("div:contains(\"[rel!='nofollow']\")")));
		assertThat(notEqualsAttributeSelectorFix.fixAttributeNotEquals("div\\\\:contains([rel!='nofollow'])"), is(equalTo("div\\\\:contains([rel!='nofollow'])")));
		...
		assertThat(notEqualsAttributeSelectorFix.fixAttributeNotEquals("div\\:contains([rel!='nofollow'])"), is(equalTo("div\\:contains(:not([rel='nofollow']))")));
	}
